...
module.exports = class HTTPClientRequest extends HTTPOutgoingMessage {
  ...
  _onresponse (head, body) {
    ...
    if (body.byteLength > 0) {
      if (isChunked) {
        const tail = Buffer.from([48, 13, 10, 13, 10])
        const crlf = Buffer.from([13, 10])

        body = body.subarray(0, body.indexOf(tail)) // cut last line

        let message = Buffer.from([])

        while (body.byteLength > 0) { // read one line per loop
          body = body.subarray(body.indexOf(crlf) + crlf.byteLength) // cut chunk size line
          message = Buffer.concat([message, body.subarray(0, body.indexOf(crlf))]) // copy message
          body = body.subarray(body.indexOf(crlf) + crlf.byteLength) // cut message line
        }
