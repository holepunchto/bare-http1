...
const { has, normalize } = require('./headers')
...
module.exports = class HTTPClientRequest extends HTTPOutgoingMessage {
  ...
  _frame(length = -1) {
    ...
    // body of unknown length reads whatever follows the headers as one.
    if (length <= 0 && this._expectsBody === false && this._writing === false) {
