...
const { get, has, normalize, single } = require('./headers')
...
module.exports = class HTTPClientRequest extends HTTPOutgoingMessage {
  constructor(opts = {}, onresponse = null) {
    ...
    // The request target of a CONNECT is the authority of the tunnel rather
    // than a path, and RFC 9110 has `Host` name that same authority: a proxy
    // that routes on the field would otherwise be told about itself. A caller
    // that named no target has no authority to mirror, so the one the socket
    // was opened to stands in.
    const target = method === 'CONNECT' && opts.path ? path : hostHeader(host, port, defaultPort)

    const fields = normalize({ host: target, ...opts.headers })
...
function announcesEmptyBody(fields) {
  ...
  const length = single(get(fields, 'content-length'))
