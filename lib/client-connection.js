  ...
  _dispatch(op) {
    switch (op.type) {
      case RESPONSE: {
        ...
        // A peer that answers of its own accord, or answers the same request
        // twice, is trying to get a response of its choosing paired up with
        ...
        // to, so it is delivered as the response it claims to be, as Node.js does.
        ...
        // A socket that is handed over to another protocol is no longer ours to
        // hand back to the agent, so the handler that does so is registered
