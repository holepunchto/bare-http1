const { isEnding, isFinished } = require('bare-stream')
const HTTPParser = require('bare-http-parser')
const HTTPConnection = require('./connection')
const HTTPIncomingMessage = require('./incoming-message')
const errors = require('./errors')
const { get, isClosing, isUpgrading } = require('./headers')

const {
  constants: { REQUEST, RESPONSE, DATA, END }
} = HTTPParser

module.exports = class HTTPClientConnection extends HTTPConnection {
  static _connections = new WeakMap()

  static from(socket, opts) {
    return this.for(socket) || new this(socket, opts)
  }

  constructor(socket, opts = {}) {
    const { IncomingMessage = HTTPIncomingMessage } = opts

    super(socket, opts)

    this._IncomingMessage = IncomingMessage

    // Whether the peer has said that it is going to close the connection, in
    // which case the socket must not go back into the agent's pool.
    this._close = false

    // How long the peer said it holds on to the connection, in milliseconds, or
    // -1 when it said nothing. A socket kept for longer than the peer keeps it
    // is one the next request goes out on just as it is being taken away.
    this._keepAliveTimeout = -1

    // Whether an interim 1xx response is being parsed, whose end does not
    // complete the request.
    this._interim = false

    // Whether the request in hand has been answered.
    this._responded = false

    this._attach()
  }

  get keepAliveTimeout() {
    return this._keepAliveTimeout
  }

  request(req) {
    this._req = req
    this._responded = false

    // A response to HEAD is framed as though it had a body but never carries
    // one, and only the request knows that.
    if (req.method === 'HEAD') this._parser.skipBody()
  }

  // The response has arrived in full, so both messages let go of the socket
  // rather than take it down with them when they are destroyed.
  _complete() {
    // A request that never finished writing would go on sending a body onto a
    // socket the peer has already moved past, so the connection cannot carry
    // another exchange.
    if (this._req !== null && isFinished(this._req) === false) this._close = true

    // Anything the peer sent past the response would be read as the answer to
    // whatever request the connection carried next.
    if (this._pending.length > 0 || this._parser.drain().byteLength > 0) this._close = true

    if (this._res !== null) {
      this._res._socket = null
      this._res._complete = true
      this._res.push(null)
    }

    if (this._req !== null) {
      this._req._socket = null
      this._req.destroy()
    }
  }

  // Cuts short whatever is in flight, reporting the error to the response if
  // there is one, as its consumer is the one waiting on it, and to the request
  // otherwise.
  _fail(err = errors.CONNECTION_LOST()) {
    const req = this._req
    const res = this._res
    const responded = this._responded

    this._req = null
    this._res = null

    this._pending = []
    this._ops = null

    // A response whose body has already arrived in full is left alone, as its
    // consumer may not have read all of it yet.
    if (res !== null && isEnding(res) === false) res._abort(err)

    // Once the request has been answered the response is what its consumer is
    // waiting on, whether it is still holding the response or has already let
    // go of it.
    if (req !== null) {
      if (responded) req.destroy()
      else req.destroy(err)
    }
  }

  _continue() {
    while (this._stopped === false) {
      let op

      // Input that cannot be parsed, or that answers a request the connection
      // is not carrying, leaves it in no state to go on from. Dispatching is
      // left outside, as what a consumer makes of a message it has been handed
      // is its own affair and so is anything it throws over one.
      try {
        op = this._pullOp()

        if (op !== null) {
          if (op.type === RESPONSE) this._expectResponse()
          // A request is not something a peer answers with, and one consumed in
          // silence would let it choose how many of the bytes that follow this
          // side reads as a body rather than as the response it is waiting for.
          else if (op.type === REQUEST) throw errors.UNEXPECTED_REQUEST()
        }
      } catch (err) {
        return this._socket.destroy(err)
      }

      if (op === null) break

      this._dispatch(op)

      if (this._backpressure) return this._pause()
    }

    if (this._stopped) return

    this._resume()

    if (this._ended) this._endInput()
  }

  _endInput() {
    this._ended = false

    let complete = false

    // A response whose body is framed by the connection closing is completed by
    // the close. Anything else still in flight has been cut short.
    try {
      for (const op of this._parser.end()) {
        if (op.type === END) complete = true
      }
    } catch {
      // Reported as a lost connection below.
    }

    if (complete) this._complete()
    else this._fail()
  }

  _dispatch(op) {
    switch (op.type) {
      case RESPONSE:
        return this._response(op)

      case DATA:
        // A push that does not fit is the signal to stop reading the socket
        // until the consumer has caught up.
        if (this._res && this._res.push(op.data) === false) this._backpressure = true
        break

      case END:
        // The end of an interim response does not complete the request.
        if (this._interim) {
          this._interim = false
          break
        }

        this._complete()
        break
    }
  }

  // A peer that answers of its own accord, or answers the same request twice, is
  // trying to get a response of its choosing paired up with whatever request
  // comes next on the connection.
  _expectResponse() {
    if (this._req === null || this._res !== null) throw errors.UNEXPECTED_RESPONSE()
  }

  _response(op) {
    const httpVersion = op.version === 'HTTP/1.0' ? '1.0' : '1.1'

    // Only a `101` switches protocols. A peer that names an upgrade alongside
    // any other status is answering the request rather than handing the
    // connection over, and must not be able to take the socket away from the
    // consumer waiting on a response, as Node.js does not let it.
    const upgrade = op.code === 101 && isUpgrading(op.headers)

    // Whatever a CONNECT is answered with is the answer about the tunnel, so
    // everything past the headers belongs to what the two ends made of it
    // rather than to HTTP, as Node.js has it. A refusal reaches the consumer
    // with the response that explains it rather than as no event at all.
    const tunnel = this._req.method === 'CONNECT'

    // A 101 that names no protocol has nothing to hand the connection over to,
    // so it is delivered as the response it claims to be, as Node.js does.
    const switching = op.code === 101 && upgrade === false

    // An interim 1xx response is not the response to the request; the real one
    // still follows, so the request is left open.
    if (
      upgrade === false &&
      tunnel === false &&
      op.code >= 100 &&
      op.code < 200 &&
      op.code !== 101
    ) {
      return this._information(op, httpVersion)
    }

    const req = this._req

    req.on('close', () => {
      // Only the request the connection is currently on has any say in this. A
      // connection that goes back into the pool may already have been handed the
      // next request by the time this one closes.
      if (this._req !== req) return

      this._req = null
    })

    const res = (this._res = new this._IncomingMessage(this._socket, {
      headers: op.headers,
      httpVersion,
      statusCode: op.code,
      statusMessage: op.reason
    }))

    this._responded = true

    res._demand = this._demand

    // The parser stops for good at a 101, so a connection that carried one
    // cannot carry another exchange and must not be offered for one.
    this._close = switching || isClosing(op.headers, httpVersion)

    this._keepAliveTimeout = keepAliveTimeout(op.headers)

    // A socket that is handed over to another protocol is no longer ours to hand
    // back to the agent, so the handler that does so is registered below rather
    // than above this.
    if (upgrade || tunnel) {
      return this._handover(this._parser.drain(), tunnel ? 'connect' : 'upgrade')
    }

    res.on('close', () => {
      // Only the response the connection is currently on has any say in what
      // becomes of the socket.
      if (this._res !== res) return

      this._res = null
      this._idle = true

      if (this._close) this._socket.end()
      else this._socket.emit('free')
    })

    req.emit('response', res)
  }

  _information(op, httpVersion) {
    const req = this._req

    this._interim = true

    // The parser spends `skipBody` on the first set of headers it completes, and
    // an interim response is not the one the request was waiting for.
    if (req.method === 'HEAD') this._parser.skipBody()

    // A 100 answers an expectation the request announced, and is what a caller
    // that is holding its body back is waiting for. Reported as an event of its
    // own, ahead of the one every interim response gets, as Node.js does.
    if (op.code === 100) req.emit('continue')

    req.emit('information', {
      httpVersion,
      statusCode: op.code,
      statusMessage: op.reason,
      headers: op.headers
    })
  }

  _handover(data, event) {
    const req = this._req
    const res = this._res
    const head = this._takeover(data)

    // The socket belongs to whatever protocol is taking it over now, so whoever
    // opened it is told to stop counting it as one of theirs.
    this._socket.emit('agentRemove')

    res._upgrade = req._upgrade = true
    res._complete = true

    const handled = req.emit(event, res, this._socket, head)

    res.push(null)
    req.destroy()

    if (!handled) this._socket.destroy()
  }

  _detach() {
    super._detach()

    HTTPClientConnection._connections.delete(this._socket)
  }

  _onerror(err) {
    this._fail(err)
  }

  _onclose() {
    // Input that was held back while the body was not being read still frames
    // the response, so it is finished off before the close is reported and the
    // rest of the body is thrown away.
    if (this._ended) {
      this._backpressure = false

      this._continue()
    }

    // No error of its own, as the close may well have been initiated locally.
    this._fail()
  }

  _ondrain() {
    if (this._req) this._req._continueWrite()
  }

  _ontimeout() {
    if (this._req) this._req.emit('timeout')
  }
}

// How long the peer says it holds an idle connection open for, in milliseconds.
// https://datatracker.ietf.org/doc/html/draft-thomson-hybi-http-timeout-03
function keepAliveTimeout(headers) {
  const value = get(headers, 'keep-alive')

  if (value === undefined) return -1

  for (const parameter of String(value).split(',')) {
    const match = /^\s*timeout\s*=\s*(\d+)\s*$/.exec(parameter)

    if (match === null) continue

    const ms = Number(match[1]) * 1000

    // A wait longer than a timer can be given is one nothing was said about, as
    // the peer is not going to reclaim the connection before this side does.
    return Number.isSafeInteger(ms) && ms <= 0x7fffffff ? ms : -1
  }

  return -1
}
