...
// How far ahead of the wait a peer announced a pooled socket is given up, so
// that one is never handed to a request in the moment the peer reclaims it.
const KEEP_ALIVE_TIMEOUT_BUFFER = 1000
...
class HTTPAgent extends EventEmitter {
  ...
  reuseSocket(socket, req) {
    ...
    // The socket was left holding the deadline it was pooled under, which is
    // one the peer may have named rather than one this side chose. A request
    // that takes the socket back is not idle, so the wait goes back to the one
    // the agent was given, and the request sets its own over that.
    socket.setTimeout(this._opts.timeout > 0 ? this._opts.timeout : 0)
  }

  keepSocketAlive(socket) {
    if (this._keepAlive === -1) return false

    const timeout = this._idleTimeout(socket)

    // The peer gives the connection up again too soon for it to be worth
    // keeping: a request handed one that is already on its way out is a request
  ...
  // How long a socket that has come free may sit in the pool: the agent's own
  // wait, held down to however long the peer said it keeps the connection.
  // Zero means the peer keeps it for too little time to be worth pooling at
  // all, and -1 that nothing bounds the wait.
  _idleTimeout(socket) {
    const timeout = this._opts.timeout > 0 ? this._opts.timeout : -1

    const connection = HTTPClientConnection.for(socket)
    const announced = connection === null ? -1 : connection.keepAliveTimeout

    if (announced === -1) return timeout

    // Given up a little ahead of the peer, as a socket handed to a request just
    // as the peer takes it away is one that request loses.
    const bounded = announced - KEEP_ALIVE_TIMEOUT_BUFFER

    if (bounded <= 0) return 0

    return timeout === -1 || bounded < timeout ? bounded : timeout
  }
