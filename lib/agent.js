...
class HTTPAgent extends EventEmitter {
  ...
  getName(opts) {
    let name = `${opts.host}:${opts.port}`

    // A connection is only interchangeable with another that would have been
    // made the same way, and the address family decides where it goes.
