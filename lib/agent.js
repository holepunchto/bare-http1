...
class HTTPAgent extends EventEmitter {
  ...
  addRequest(req, opts) {
    ...
    if (socket) this.reuseSocket(socket, req)
      ...
      function ontimeout() {
        // A socket that is in use belongs to whoever is using it, and the
        // timeout is theirs to act on. Only an idle one is reclaimed here,
        // since nobody else is listening for it.
        if (agent._freeSockets.delete(name, socket)) socket.destroy()
      }

      // Only newly created sockets are added, as a reused socket is already
      // tracked from when it was created.
