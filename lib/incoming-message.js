...
module.exports = class HTTPIncomingMessage extends HTTPMessage(Readable) {
  constructor(socket = null, opts = {}) {
    ...
    // Whether the whole of the message arrived, as against the peer having gone
    // away part way through it. The stream closes either way, so this is what
    // tells a consumer which of the two it was handed.
  ...
  // Cuts the message short. The failure is only reported as an error if someone
  // is there to hear it, as Node.js does, since an unhandled `error` would take
  // the process down instead. A consumer with nobody listening still has
  // `complete` to tell it that the message it was handed stops short.
  _abort(err = null) {
    if (err !== null && this.listenerCount('error') > 0) this.destroy(err)
    else this.destroy()
  }
