...
module.exports = class HTTPServerConnection extends HTTPConnection {
  ...
  constructor(server, socket, opts = {}) {
    ...
    // Whether anything has been read since the headers were last waited for,
    // which tells a peer that is slow about sending a request apart from one
    // that is simply not sending another.
    this._partial = false
  ...
  _abort(err = null) {
    ...
    if (req === null || isEnded(req) || req.destroying) return
    ...
    // does, since an unhandled `error` would take the process down instead. A
    // consumer with nobody listening still has `complete` to tell it that the
    // message it was handed stops short.
    if (err !== null && req.listenerCount('error') > 0) req.destroy(err)
    else req.destroy()
  }
  ...
  // Gives up on a connection that is closing, so that whatever the peer sent
  // behind the last message it will be answered is dropped rather than acted on.
  _hangUp() {
    this._stop()
  ...
  _dispatch(op) {
    switch (op.type) {
      ...
      case DATA:
        ...
        // whole rather than stalling the handover.
  ...
  _headersTimeout() {
    // Nothing was being waited for beyond another request, so this is an idle
    // connection being reclaimed rather than a request going unfinished.
    if (this._partial === false) return this._keepAliveTimeout()
