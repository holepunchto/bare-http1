...
const { has, hasToken, isClosing, isUpgrading } = require('./headers')
...
module.exports = class HTTPServerConnection extends HTTPConnection {
  ...
  _dispatch(op) {
    switch (op.type) {
      ...
      // The parser reads whichever kind of message the input looks like, and a
      // response is not one a server can answer. Left alone it would be
      // consumed in silence, declared body and all, so where this side thinks
      // the next message begins would rest on bytes no request could carry.
      case RESPONSE:
        return this._fail(
          errors.UNEXPECTED_RESPONSE('Response received in place of a request'),
          BAD_REQUEST
        )
  ...
  _request(op) {
    ...
    if (isUpgrading(op.headers) && this._server.listenerCount('upgrade') > 0) {
      const expect = req.httpVersion === '1.0' ? undefined : op.headers.expect

      if (expect === undefined || hasToken(expect, '100-continue')) {
        const announced = op.headers['content-length']

        // The handover waits for the whole request, and a client that announced
        // an expectation holds its body back until it is answered, so the two
        // ends would be left waiting on each other. Only a request that
        // announced a body has one being held back; a 100 for any other would
        // only get in the way of the response that hands the connection over.
        if (
          expect !== undefined &&
          (announced !== undefined || has(op.headers, 'transfer-encoding'))
        ) {
      ...
      // An expectation this side cannot meet is left to the ordinary path,
      // which refuses it rather than hand the connection over to a protocol
      // whose first bytes are a body that is never going to arrive.
    }
