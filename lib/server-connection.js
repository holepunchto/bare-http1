    ...
    switch (op.type) {
      case REQUEST: {
        ...
        if (op.method === 'CONNECT') {
          return this._handover(this._parser.drain(), 'connect')
        }
        ...
        if (has(op.headers, 'upgrade') && hasToken(op.headers.connection, 'upgrade')) {
          return this._handover(this._parser.drain(), 'upgrade')
        }
        ...
        // A client that announced an expectation may be holding its body back
        // until it has been answered. An HTTP/1.0 client is left out, as RFC
        // 9110 gives it no way to understand a 1xx and it would read the 100 as
        // the response itself.
        if (op.headers.expect !== undefined && req.httpVersion !== '1.0') {
          if (hasToken(op.headers.expect, '100-continue')) {
            if (this._server.listenerCount('checkContinue') > 0) {
              this._server.emit('checkContinue', req, res)
            } else {
              res.writeContinue()
            ...
            this._server.emit('checkExpectation', req, res)
          } else {
            // Nothing here understands the expectation, and RFC 9110 gives the
            // peer no way to go on without one, so the request is refused.
            res.writeHead(417)
            res.end()
          }
