...
module.exports = class HTTPServerConnection {
  ...
  _fail(err, response) {
    ...
    if (this._server.emit('clientError', err, this._socket) === false) {
      ...
      // written to any more.
      if (isWritable(this._socket)) this._socket.write(response)
    }
  ...
  _onop(op) {
    switch (op.type) {
      case REQUEST: {
        ...
        this._req._ondemand = this._ondemand
      ...
      case END:
        ...
        // The request is in, so the peer is only being waited on for the next
        // one, if it sends one at all.
        this._requestDeadline.disarm()
        this._partial = false
        this._headersDeadline.arm()
        break
