...
module.exports = class HTTPServerConnection {
  constructor (server, socket, opts = {}) {
    ...
    this.ondrain = this._ondrain.bind(this)
  ...
  _onhead (data) {
    ...
    if (headers.connection?.toLowerCase() === 'upgrade') this._upgrading = true
    else this.server.emit('request', this.req, this.res)

    if (headers['transfer-encoding'] === 'chunked') {
      this._state = constants.state.BEFORE_CHUNK
    } else if (this._upgrading) {
      this._onupgrade(this._buffer)
    } else {
  ...
  _onchunklength (data) {
    ...
    if (this._length === 0) {
      if (this._upgrading) this._onupgrade(null)
      else this._onfinished()
    } else {
      this._state = constants.state.IN_CHUNK
    }
  }

  _onchunk (data) {
    if (this._upgrading) return this._onupgrade(data)
  ...
  _onupgrade (data) {
    this.socket
      ...
      .off('data', this.ondata)
      .off('drain', this.ondrain)

    this.server.emit('upgrade', this.req, this.socket, data || Buffer.alloc(0))
  }
