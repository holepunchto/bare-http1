...
module.exports = class HTTPServerConnection {
  ...
  constructor(server, socket, opts = {}) {
    ...
    // Between requests there is nothing being waited on but the next one, which
    // is worth far less patience than an unfinished request: the peer is not
    // mid-anything and a connection it has no more use for is only holding
    // resources here.
    this._keepAliveDeadline = new Deadline(server.keepAliveTimeout, () =>
  ...
  _abort(err = null) {
    ...
    // was queued for the peer. The socket itself stays reachable, since
    // `req.socket` is how a consumer asks who it is talking to.
  ...
  _continue() {
    while (this._failed === false && this._detached === false && this._closed === false) {
      if (this._op !== null) {
  ...
  _onop(op) {
    switch (op.type) {
      case REQUEST: {
        ...
        this._res.on('error', this._onresponseerror)

        this._res.on('close', () => {
          this._res = null

          // Whatever is left of the request body still has to come off the
          // socket..
          this._drain()
        ...
        // A client that announced an expectation has to be answered on its own
        // terms, as it may be holding its body back until it has been. An
        // HTTP/1.0 client is left out of all of this, as RFC 9110 gives it no
        // way to understand a 1xx and it would read the 100 as the response
        // itself.
        if (op.headers.expect !== undefined && this._req.httpVersion !== '1.0') {
          if (hasToken(op.headers.expect, '100-continue')) {
            if (this._server.listenerCount('checkContinue') > 0) {
              this._server.emit('checkContinue', this._req, this._res)
            } else {
              this._res.writeContinue()

              this._server.emit('request', this._req, this._res)
            }
          } else if (this._server.listenerCount('checkExpectation') > 0) {
            this._server.emit('checkExpectation', this._req, this._res)
          } else {
            // Nothing here understands the expectation, and RFC 9110 gives a
            // peer no way to go on without one it asked for, so the request is
            // refused rather than handled as though it had not asked.
            this._res.writeHead(417)
            this._res.end()
          }
  ...
  _onidle() {
    ...
    // A pipelined request that is only waiting for the connection to free up
    // is already in hand and about to be answered
    ...
    // Nothing is left to wait for but the next request, if the peer sends one at
    // all. The deadline is only started once, as going idle a second time for
    // the same request, when its consumer catches up, must not buy the peer any
    // more time.
  ...
  _onheaderstimeout() {
    if (this._partial === false) {
      ...
      return destroySoon(this._socket)
    }
  ...
  _pause() {
    ...
    this._keepAliveDeadline.suspend()
  }
  ...
  _resume() {
    ...
    this._keepAliveDeadline.resume()
  }
