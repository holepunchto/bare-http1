...
function HTTPOutgoingMessage(Stream) {
  return class HTTPOutgoingMessage extends HTTPMessage(Stream) {
    constructor(socket = null) {
      ...
      // Whether a body is on its way, which is what tells a message that would
      // otherwise carry none apart from one that is only having its headers
      // sent ahead of time.
      this._writing = false
    ...
    write(data, encoding, cb) {
      if (this._headersSent === false && this._socket !== null && this._error === null) {
        // The headers have to frame a body, however little is known about it
        // yet, as one is on its way.
        this._writing = true
    ...
    _reframe() {
      ...
      const length = headers.single(headers.get(this._headers, 'content-length'))
    ...
    _frameBodyEmpty() {
      ...
      // Nothing having been written is a body of no length, so an announced one
      // that asks the peer to read further is caught here rather than after the
      // headers promising it have gone out.
      if (this._length > 0 && this._discardBody() === false) {
        throw this._mismatch(0)
      }
    ...
    _write(data, encoding, cb) {
      ...
      // Anything that is not bytes has no length to frame the message with, and
      // would otherwise be announced as a body of `NaN` bytes.
      if (ArrayBuffer.isView(data) === false) {
        return cb(errors.INVALID_BODY(`Body must be a string or a view, got ${typeof data}`))
      }
